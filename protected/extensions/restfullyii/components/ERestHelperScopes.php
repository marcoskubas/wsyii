...
class ERestHelperScopes extends CActiveRecordBehavior {
    ...
    public function fieldBy($field){
        if(!empty($field)){
            $this->Owner->getDbCriteria()->mergeWith(array(
                'select' => Functions::JSONdecode($field)
            ));
        }
        return $this->Owner;
    }
    ...
    public function orderBy($field, $dir = 'ASC') {
        ...
        if (!is_array($orderListItems = CJSON::decode($field))) {
            ...
            $orderByStr = "";
            $orderListItems = Functions::JSONdecode($field); //Implements Metacem
            ...
            if (!is_null($filterItem['property'])) {
                ...
                if (array_key_exists('operator', $filterItem) || is_array($value)) {
                    ...
                    switch ($operator) {
                        ...
                        case 'like':
                            $compare = " LIKE :" . $prop;
                            $params[(":" . $prop)] = $value; //Implements Metacem (retirado %$value%)
                            break;
                        case '=' :
                        case '<' :
                        case '<=':
    ...
    private function getFilterAlias($property) {
        if(in_array($property, $this->Owner->metaData->columns))
            return $this->Owner->getTableAlias(false, false) . '.';
    }

    private function getSortSQL($field, $dir = 'ASC') {
        if(!isset($this->Owner->metaData->columns[$field])){
            return "$field $dir"; //Implements Metacem
        }else{
